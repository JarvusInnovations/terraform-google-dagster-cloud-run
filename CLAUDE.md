...
- OpenTofu (`tofu`), managed via asdf; run `tofu fmt -recursive` before committing
...
- **Cross-repo plans** (marked in their Scope) execute in other repos
  (gtfs-realtime-archiver, a second origin deployment's private repo,
  dagster-io/community-integrations) and must not be auto-executed by parallel
  worktree agents.
